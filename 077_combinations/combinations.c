...
static void recursive(int n, int k, int start, int *stack, int len, bool *used,
                      int *col_sizes, int **results, int *count)
{
    int i;
    if (len == k) {
        col_sizes[*count] = k;
        ...
        for (i = start; i <= n; i++) {
            if (!used[i]) {
                stack[len] = i;
                used[i] = true;
                recursive(n, k, i + 1, stack, len + 1, used, col_sizes, results, count);
                used[i] = false;
            }
        }
    }
...
int** combine(int n, int k, int** columnSizes, int* returnSize) {
    ...
    *columnSizes = malloc(capacity * sizeof(int));
    recursive(n, k, 1, stack, 0, used, *columnSizes, results, &count);
    *returnSize = count;
    return results;
}
